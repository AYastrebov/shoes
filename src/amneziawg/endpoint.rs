...
impl EndpointSocket {
    ...
    /// belongs to the path rather than to the peer — the AmneziaWG 3.1
    /// trailer window, and the announcement that tells the server the new
    /// address (both in `tunnel.rs`). It only queues; the tunnel's own send
