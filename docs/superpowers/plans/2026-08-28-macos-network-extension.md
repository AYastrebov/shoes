...
- Every Apple slice builds with `--profile release-mobile --features control-stats`; the macOS slice adds `ffi`, which is what compiles the FFI module there. `control-logs` stays off.
...
| `.github/workflows/mobile.yml` `macos` job | 1, 2 | rename; FFI test step |
...
### Task 1: Name the macOS no-op, and the job that checks the surface

**Files:**
- Modify: `src/tun/tun_server.rs:276-281`
- Modify: `.github/workflows/mobile.yml:81-83`

**Interfaces:**
- Produces: nothing later tasks consume. This task records two facts the rest of the work relies on.

- [ ] **Step 1: Confirm the default-features build already compiles**

Run: `$CARGO_PATH cargo check --target aarch64-apple-darwin 2>&1 | tail -1`
Expected: `Finished`. The FFI module is behind `--features ffi` on macOS by design (`src/lib.rs:135`), so nothing to fix; Task 3 builds the macOS slice with that feature.

- [ ] **Step 2: Explain the dead builder field**
...
- [ ] **Step 3: Rename the CI job**

In `.github/workflows/mobile.yml`, the `macos` job's `name: Build macOS staticlib` becomes `name: Check the macOS C surface` — the verification step deliberately reads the dylib, and the name said otherwise.
...
```bash
git add src/tun/tun_server.rs .github/workflows/mobile.yml
git commit -m "tun: say why the macOS packet_information field is a no-op"
```
